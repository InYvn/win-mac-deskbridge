...
现在，你的双端键位已经高度统一。但在实际使用中，你可能会遇到一个**新的痛点**：

❌ **输入法冲突**：当你把鼠标移到 Windows 上打字时，如果 Mac 端还停留在**中文输入法**，Windows 上可能无法正常输入符号，因为 Mac 拦截了键盘信号。

为了实现“去 Windows 自动切英文，回 Mac 自动恢复”的丝滑体验，请前往下一节：

**[⚡️ 进阶：输入法自动切换](./2-1-input-fix.md)**